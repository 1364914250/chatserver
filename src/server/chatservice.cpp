...
void ChatService::login(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if (user.getId() == id && user.getPassword() == password)
    {
        ...
        else
        {
            ...
            // 查询用户的好友信息并返回
            vector<User> userVec = _friednMode.query(id);
            if (!userVec.empty())
            {
                vector<string> vec2;
...
void ChatService::createGroup(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if (_groupMode.createGroup(group))
    {
        // 存储创建人信息
        _groupMode.addGroup(userid, group.getId(), "creator");
    }
}
// 加入群组业务
void ChatService::addGroup(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    _groupMode.addGroup(userid, groupid, "normal");
}
// 群组聊天业务
void ChatService::groupChat(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    int userid = js["id"].get<int>();
    int groupid = js["groupid"].get<int>();
    vector<int> useridVec = _groupMode.queryGroupUsers(userid, groupid);
...
void ChatService::addFriend(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    // 存储好友信息
    _friednMode.insert(userid, friendid);
}
